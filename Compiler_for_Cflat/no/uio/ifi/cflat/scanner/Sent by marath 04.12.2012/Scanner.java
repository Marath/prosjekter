...
public class Scanner {
	...
	while (nextNextToken == null) {
		...
		/** Symbolbehandling -----------*/
	    } else if (isSymbol(CharGenerator.curC)/* || isSymbol(CharGenerator.nextC)*/ || (!isNumber09(CharGenerator.nextC) && CharGenerator.curC ==  '-')) {
		...
		if(!isFeed(CharGenerator.curC)) {
			...
			case "-": nextNextToken = subtractToken;break;
		...
		/** tallbehandling ------ */
	    } else if (isNumber09(CharGenerator.curC) || (CharGenerator.curC == '-' && isNumber09(CharGenerator.nextC))) {
		dummy = "";
	...
	private static boolean isSymbol(char c) {
	    ...
	    if((ch >= 33 && ch <= 44) || (ch == 47) || (ch >= 58 && ch <= 62) || (ch >= 91 && ch <=93) || ch == 96 || (ch >= 123 && ch <=125)) 
		return true;
	    return false;
	}
