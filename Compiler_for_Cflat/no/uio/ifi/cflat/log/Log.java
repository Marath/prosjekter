...
public class Log {
    ...
    public static void indentParser(String symbol) {
	String indent = "";
	...
	try {
	    PrintWriter log = (nLogLines==0 ? new PrintWriter(logName) :
			       new PrintWriter(new FileOutputStream(logName,true)));
	    //log.print("Parser:" + indent);
	    log.println("Parser:   " + indent + symbol);  ++nLogLines;
	    log.close();
	} catch (FileNotFoundException e) {
	    Error.error("Cannot open log file " + logName + "!");
	}
    ...
    // useLineNum - where its used, lineNum - where it was 'declared'
    public static void noteBinding(String name, int lineNum, int useLineNum) {
	if (! doLogBinding) return;
	//-- Must be changed in part 2:
	if(name.equals("main")) {
	    writeLogLine("Binding: " + name + " refers to declaration in line " + useLineNum);
	}
	else if(useLineNum == 0) {
	    writeLogLine("Binding: Line " + lineNum + ": " + name + " referes to declaration in the library");
	}
	else {
	    writeLogLine("Binding: Line " + lineNum + ": " + name + " referes to declaration in line " + useLineNum);
	}
    }
