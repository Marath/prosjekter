...
class FuncDecl extends Declaration {
	...
	pdl.genCode(this);
	fb.genCode(this);
	if(type.typeName().equals(Types.doubleType.typeName())) {
	    Code.genInstr("", "fldz", "", "");
	}
...
class CallStatm extends Statement {
	...
	//-- Must be changed in part 2:
	fc.genCode(curFunc);
	if(fc.valType.typeName().equals(Types.doubleType.typeName())) {
...
class IfStatm extends Statement {
	...
	if(e == null) {
	    ...
	    System.err.println("lab from if: " + lab);
	    test.genCode(curFunc);
	    ...
	    System.err.println("lab1 = " + lab1 + "\tlab2 = " + lab2);
	    test.genCode(curFunc);
...
class ExprList extends SyntaxUnit {
	...
	System.out.println("in function: " + curFunc.name);
	System.out.println("paramNum: " + paramNum + "\t" + cur.valType.typeName() + ".equals(" +Types.doubleType.typeName() + ")");
	// System.out.println("relOp =  " + cur.relOp);
    	cur.genCode(curFunc);
	//if(curFunc.type.typeName().equals(Types.doubleType.typeName())) {
	if(cur.valType.typeName().equals(Types.doubleType.typeName())) {
	    //System.out.println("param " + paramNum + " is a double");
	    Code.genInstr("", "subl", "$8,%esp", "");
	    Code.genInstr("", "fstpl", "(%esp)", "Push parameter #" + paramNum);
	}
	else if(cur.valType.typeName().equals(Types.intType.typeName())) {
	    //System.out.println("param " + paramNum + " is an int");
	    //System.out.println("relOp =  " + cur.relOp);
	    Code.genInstr("", "pushl", "%eax", "Push parameter #" + paramNum);
	}
	else {
	    Syntax.error(this, "Expression should have int or double type");
	}
    }
}
...
class Expression extends Operand {
	...
	if(secondTerm != null) {
	    ...
	    if(secondTerm.type.typeName().equals(firstTerm.type.typeName())) {
		...
		//     System.err.println("\n-in line: " + lineNum + " Type is doubletype");
		//     System.err.println("FROM EXPRESSION IN IF: In line: " + lineNum + " type.size() = " + valType.size() + " ---------------_ ");
		// } else if(valType == Types.intType) {
		//     System.err.println("\n-in line: " + lineNum + " Type is inttype");
		//     System.err.println("FROM EXPRESSION IN IF: In line: " + lineNum + " type.size() = " + valType.size() + " ---------------_ ");
		// }
	    ...
	    // if(valType == Types.doubleType) {
	    // 	 System.err.println("\n-in line: " + lineNum + " Type is doubletype");
	    // 	 System.err.println("FROM EXPRESSION IN ELSE: In line: " + lineNum + " type.size() = " + valType.size() + " ---------------_ ");	 
	    //  } else if(valType == Types.intType) {
	    // 	 System.err.println("\n-in line: " + lineNum + " Type is inttype");
	    // 	     System.err.println("FROM EXPRESSION IN ELSE: In line: " + lineNum + " type.size() = " + valType.size() + " ---------------_ ");
	    // }
	 ...
	 if(relOp != null) {
	     ...
	     //System.out.println("RelOp is not NULL /////////////////////////////////////////////////////////////////////////");
	     if (relOp.opType.typeName().equals(Types.doubleType.typeName())) {
		 //System.out.println("in expression " + valType.typeName() + ".equals(" + Types.doubleType.typeName() + ")");
             // if (valType.typeName().equals(Types.doubleType.typeName())) {
		 //Code.genInstr("", "subl", "8(%esp)", "");
		 System.out.println("expression is double");
		 Code.genInstr("", "subl", "$8,%esp", "");
		 Code.genInstr("", "fstpl", "(%esp)", "");
	     } 
	     else {
		 System.out.println("expressiom is int");
		 Code.genInstr("", "pushl", "%eax", "");
	     }
 ...
 class Term extends SyntaxUnit { 
	 ...
	 while(otmp != null) {
	     ...
	     //if(otmp.opType == Types.doubleType) {
 ...
 class FactorList extends SyntaxUnit {
	 ...
	 // first set type to firstFcator type
...
class Number extends Operand {
	...
	valType = Types.getType(intToken);
...
class Variable extends Operand {
	...
	if (index == null) {
	    ...
	    valType = d.type;
	    System.err.println("-varName from Variable: " + varName);
	} else {
